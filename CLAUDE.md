...
- The site's `#weekSelect` dropdown always offers the current week plus one week ahead; `fetchWithSelenide` (in `Browser.kt`) scrapes the current week, then switches the dropdown to the next option and re-scrapes, returning both HTML sources as a `List<String>`. `Main.kt` parses each source and flattens the results, so both weeks' days get diffed and written under `.days/`.
...
- `Notifier.kt` — posts plain-text messages to the Telegram Bot API using `java.net.http.HttpClient`. Reads `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` from env and is a silent no-op if either is unset, so local runs and CI without secrets don't fail. `TELEGRAM_CHAT_ID` accepts a comma-separated list — every configured chat gets the same message, and failures for individual recipients are collected and reported together so one broken id doesn't block the others. Messages are truncated to 4000 chars (Telegram's limit is 4096). The bot is a one-way pusher: it does not poll `getUpdates`, so it will never reply to `/start` or other incoming messages — a user sending `/start` is still required though, since Telegram blocks bots from DM'ing users who haven't initiated contact.
